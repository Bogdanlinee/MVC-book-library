...
var msgResultSearchText = function (text, number_found) {
    ...
    var coincidence = titles[(number_found % 100 > 4 && number_found % 100 < 20) ? 2 : cases[(number_found % 10 < 5) ? number_found % 10 : 5]];
...
var requestBooksSearch = function (callback) {
    ...
    if (text.length > 0) {
        ...
        // doAjaxQuery('GET', '' + pathUrl + '/api/v1/books?search=' + textEncode + '', null, function (res) {
        //     callback(res, text);
        // });
...
$('#search').keyup(function (event) {
    var text = $(this).val();
    items_search_on_page_load = text;
    ...
    if (event.keyCode === 13) {
        if (text.length > 0) {
            ...
            if (pathUrl == '/admin') {
                ...
                // var url = 'http://' + window.location.host + pathUrl + '/api/v1/books?search=' + encodeText + '';
                // window.location = url;
                addBookListOnThePage();
            }
        }
    ...
    if (text.length > 0) {
        if (!(event.keyCode >= 33 && event.keyCode <= 40) && !(event.keyCode >= 16 && event.keyCode <= 20) && (event.keyCode !== 27) && (event.keyCode !== 13)) {
